...
import type { CFAResponse } from "../../lupong-tagapamayapa-api/CFA";
      ...
      new TableRow({
        children: [
          new TableCell({
            ...
            children: [
              para(
                [run(data.luponChairman.toUpperCase(), { bold: true, size: 10 })],
    ...
    sections: [
      {
        ...
        children: [
          ...
          para(
            [run(`Usaping Barangay Blg: ${data.controlNumber}`, { bold: true })],
            { align: AlignmentType.RIGHT },
          ),
          para(
            [run(`Usaping Inihain: ${data.matterFiled.toUpperCase()}`, { bold: true })],
            { align: AlignmentType.RIGHT, spaceAfter: 240 },
          ),

          para([run(data.complinantName.toUpperCase(), { bold: true, underline: true })]),
          para([run(data.complinantAddress.toUpperCase(), { underline: true })]),
          para([run("(Mga) May Sumbong", { italic: true })], { spaceAfter: 160 }),

          para([run("Laban kay/kina:")]),
          para([run(data.respondentName.toUpperCase(), { bold: true, underline: true })]),
          para([run(data.respondentAddress.toUpperCase(), { underline: true })]),
          para([run("(Mga) Ipinagsumbong", { italic: true })], { spaceAfter: 280 }),
          ...
          new Paragraph({
            ...
            children: [
              run("Dumalo ang magkabilang panig sa Punong Barangay at nagbigay ng kanya kanyang salaysay ngunit walang naganap na pagkakasundo."),
            ],
          }),
          new Paragraph({
            numbering: { reference: "cfa-items", level: 0 },
          ...
          new Paragraph({
            ...
            children: [
              run("Dahil dito, ang kaukulang demanda para sa usaping ito ay maaari nang idulog sa hukuman o alin mang mataas na tanggapan ng pamahalaan."),
            ],
          }),
