...
export const ActionModal = ({
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm py-8 px-6 text-center animate-in zoom-in-95 duration-200">
        {/* Icon Circle */}
        <div
          className={`w-20 h-20 ${iconBg} rounded-full flex items-center justify-center mx-auto mb-5 shadow-lg`}
        >
          {icon}
        </div>
        ...
        {/* Message */}
        <div className="text-gray-600 text-sm mb-8">{children}</div>

        {/* Button */}
        <button
          onClick={onClose}
          className={`w-full py-2.5 font-semibold rounded-lg transition-all duration-200 ${buttonStyle}`}
        >
          Continue
        </button>
      </div>
