...
import  { type EmployeeTablDTO,getEmployee } from "../../blotter-api/ForwardToLupon";
  ...
  return (
    <div className="fixed inset-0 bg-gray-500/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-sm py-8 px-6 text-center">
        ...
        <button onClick={onClose} className={`w-full py-2.5 font-medium rounded transition-colors ${buttonStyle}`}>
          OK
        </button>
      </div>
    </div>
...
export function ReferToLuponModal({
  ...
  const [employees, setEmployees]       = useState<EmployeeTablDTO[]>([])
  ...
  const optionsFor = (position: string) =>
    employees.filter((e) => e.position.toLowerCase() === position.toLowerCase())
  ...
  const handleSubmit = async () => {
    ...
    if (uniqueIds.size !== selected.length) {
      setError("Duplicate selection detected. Please select unique individuals.")
      return
    }
        ...
        >
          <div className="bg-white rounded-lg shadow-lg w-full max-w-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-5 space-y-5">
              ...
              <div className="bg-gray-50 rounded-lg p-4 border border-gray-200">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  The complaint filed at the blotter level remained unresolved. Escalate this case by assigning the three (3) Pangkat Tagapagkasundo members who will handle the formal conciliation.
                </p>
              </div>
                ...
                <div>
                  ...
                  <div className="border border-gray-200 rounded-lg divide-y divide-gray-100">
                    {SLOTS.map(({ position, sublabel }, idx) => {
                      ...
                      return (
                        <div key={position} className="flex items-center justify-between px-4 py-3 gap-4">
                          ...
                          <div className="w-2/3">
                            <select
                              value={currentValue}
                              onChange={(e) =>
                                updateSelected(idx, e.target.value === "" ? "" : Number(e.target.value))
                              }
                              ...
                              style={currentValue === "" ? { color: "#9ca3af" } : {}}
                            >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-t border-gray-200 sticky bottom-0 bg-white">
              <p className="text-xs text-gray-400 font-medium">3 unique members required</p>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={isLoading || !allFilled || fetchLoading || !!fetchError}
