...
const WORKING_SESSIONS = [
  { label: 'Morning Session', start: '07:00', end: '11:30', display: '7:00 AM – 11:30 AM' },
  { label: 'Afternoon Session', start: '13:00', end: '17:00', display: '1:00 PM – 5:00 PM' },
]
...
const TIME_MESSAGES: Record<Exclude<TimeStatus, null | 'valid'>, string> = {
  ...
  crosses_lunch: 'Schedule crosses the lunch break (11:30 AM – 1:00 PM). Please split into two sessions.',
  outside: 'Selected time is outside office working hours (7:00 AM – 11:30 AM or 1:00 PM – 5:00 PM).',
}
    ...
    >
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden max-h-[95vh] flex flex-col">
        ...
        <div className="flex flex-col md:flex-row overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-2">
                {WORKING_SESSIONS.map(s => (
                  <div key={s.label} className="bg-white border border-slate-200 rounded-lg px-3 py-2">
                    ...
                    <p className="text-xs font-bold text-slate-700 mt-0.5">{s.display}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* ── Right: Form ── */}
          <div className="w-full md:w-72 p-5 space-y-4 bg-gray-50/30 shrink-0">

            {/* Selected Date */}
            <div className={`rounded-xl border p-3.5 transition-colors ${selectedDate ? 'bg-blue-50 border-blue-100' : 'bg-white border-gray-200'}`}>
              {selectedDate ? (
                <>
                  <p className="text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-1">Selected Date</p>
                  <p className="text-sm font-bold text-blue-800 leading-snug">{formattedDate}</p>
                </>
              ) : (
            ...
            {selectedDate && (
              busySlots.length > 0 ? (
                <div>
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Conflicting Schedules</p>
                  <div className="space-y-1.5">
                    {busySlots.map((slot, i) => (
                      <div key={i} className="bg-rose-50 border border-rose-100 rounded-lg px-3 py-2">
                        ...
                        <p className="text-[10px] text-rose-500 truncate mt-0.5">{slot.caseNumber} · {slot.natureOfComplaint}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            {/* Time Range */}
            <div>
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Time Range *</p>
              <div className="flex items-center gap-2">
                <input
                  type="time"
                  value={startTime}
                  onChange={e => setStartTime(e.target.value)}
                  className="flex-1 px-2 py-2 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white text-gray-800"
                />
              ...
              {timeStatus && timeStatus !== 'valid' && (
                <div className="mt-2 flex items-start gap-1.5 p-2.5 bg-amber-50 border border-amber-200 rounded-lg">
                  <AlertTriangleIcon className="w-3 h-3 text-amber-500 shrink-0 mt-0.5" />
                  <p className="text-[10px] text-amber-700 font-medium leading-snug">{TIME_MESSAGES[timeStatus]}</p>
                </div>
              )}
              {timeStatus === 'valid' && (
                <div className="mt-2 flex items-center gap-1.5 p-2.5 bg-emerald-50 border border-emerald-200 rounded-lg">
                  <CheckCircleIcon className="w-3 h-3 text-emerald-500 shrink-0" />
                  <p className="text-[10px] text-emerald-700 font-medium">Within office hours</p>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="relative">
                <select
                  value={venueOption}
                  onChange={e => { setVenueOption(e.target.value); setCustomVenue('') }}
                  className="w-full appearance-none px-3 py-2.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white text-gray-800 pr-8"
                >
                  {VENUE_OPTIONS.map(v => (
              ...
              {isCustomVenue && (
                <input
                  ...
                  className="mt-2 w-full px-3 py-2.5 text-xs border border-blue-200 bg-blue-50/30 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 text-gray-800"
                  autoFocus
                />
              )}
            </div>
            ...
            {/* Notes */}
            <div>
              <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-2">Notes (Optional)</p>
              <textarea
                value={notes}
                onChange={e => setNotes(e.target.value)}
                rows={2}
                placeholder="Additional instructions for the parties..."
                className="w-full px-3 py-2.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 resize-none bg-white text-gray-800"
              />
            </div>
            ...
            <button
              ...
              className="w-full py-2.5 text-xs font-bold bg-blue-600 text-white rounded-xl hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all shadow-sm shadow-blue-200"
            >
              {loading && (
