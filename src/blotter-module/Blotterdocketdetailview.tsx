...
import { HearingMinutesModal } from "../blotter-module/modal/HearingMinutesModal";
...
const handleOpenHearingDetails = async (hId: number, mode: "viewMinutes" | "recordMinutes") => {
...
      await updateCaseStatus({ blotterNumber: blotterNumber, newStatus, reason });
...
            onConfirm={(reason) => handleAction("SETTLED", reason)}
...
            onConfirm={(reason) => handleAction("DISMISSED", reason)}
...
  <RecordMinutesModal  // <--- DAPAT ITO ANG TATAWAGIN, HINDI 'HearingMinutesModal'
...
      await refreshData(); // Para mag-update ang status sa listahan
...
        {/* Iba pang tabs (notes, timeline) - same logic */}
