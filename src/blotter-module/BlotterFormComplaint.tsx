...
function getLoggedInFullName(): string {
  const payload = getJwtPayload();
  return payload?.fullName ?? "Unknown Officer";
}

// ─── Blotter Number API ───────────────────────────────────────────────────────
// TODO: Replace the URL below with the correct endpoint once confirmed.
const BASE = import.meta.env.VITE_API_BASE_URL ?? "http://localhost:8080";

async function fetchNextBlotterNumber(): Promise<string> {
  try {
    const token = localStorage.getItem("token");
    const res = await fetch(`${BASE}/api/v1/blotter/next-blotter-number`, {
      headers: {
        "Content-Type": "application/json",
        ...(token && { Authorization: `Bearer ${token}` }),
      },
    });
    if (!res.ok) throw new Error("Failed to fetch blotter number");
...
export default function BlotterEntryForm() {
  ...
  // ── Respondent – shared states (used in both modes) ──
  ...
  // ── Respondent – Formal (additional fields) ──
  ...
  // ── Docket info ──
  const [blotterNumber, setBlotterNumber] = useState<string>("Loading...");
  const officerName = getLoggedInFullName();
  ...
  useEffect(() => {
    fetchNextBlotterNumber().then(setBlotterNumber);
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setSuccessBlotterNo(blotterNumber);
    ...
    <div className="min-h-screen">
      <div className="max-w-6xl mx-auto px-6 py-5 flex flex-col gap-5">
        ...
        <SectionCard letter="B" title="Complainant Information">
          ...
          <FormRow cols={3}>
            <FormInput
              ...
              maxLength={11}
              value={cContact}
              onChange={(e) => { const v = e.target.value.replace(/\D/g, ""); setCContact(v); clearErr("cContact"); }}
        ...
        {mode === "record" ? (
          ...
          >
            {/* ── CHANGED: Replaced single Full Name field with Last Name, First Name, Middle Name ── */}
            <FormRow cols={3}>
              <FormInput
                id="field-rLastName"
                label="Last Name"
          ...
          >
            ...
            <FormRow cols={4}>
              ...
              <FormInput
                ...
                onChange={(e) => {
                  ...
                  setRDob(""); // clear DOB when age is manually typed
                }}
              />
              <FormDatePicker
                label="Date of Birth"
            ...
            <FormRow cols={3}>
              ...
              <FormInput
                ...
                onChange={(e) => setRFormalContact(e.target.value.replace(/\D/g, ""))}
              />
            </FormRow>
            <FormInput
              label="Complete Address"
