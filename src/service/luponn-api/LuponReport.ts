// Compatibility shim for legacy/typo import path.
export * from "../lupon-api/LuponReport";
...
export async function getTopNature(
  startDate?: string,
  endDate?: string,
): Promise<NatureReportDTO[]> {
  const query = new URLSearchParams();
  if (startDate) query.set("startDate", startDate);
  if (endDate) query.set("endDate", endDate);

  return apiFetch<NatureReportDTO[]>(
    `${REPORTS_URL}/top-nature?${query.toString()}`,
  );
}
...
export async function getCasesTrend(
  ...
  return apiFetch<ChartDataDTO[]>(
    `${REPORTS_URL}/cases-trend?${query.toString()}`,
  );
}

export async function getMonthlyDilgReport(
  month: number,
  year: number,
): Promise<LuponMonthlyReportDTO[]> {
  const query = new URLSearchParams();
  query.set("month", String(month));
  query.set("year", String(year));

  return apiFetch<LuponMonthlyReportDTO[]>(
    `${REPORTS_URL}/monthly-dilg?${query.toString()}`,
  );
}
