...
export interface RecordMinutesViewDTO {
  ...
  respondentPresent: boolean;
  chairmanPresent: boolean | null;
  secretaryPresent: boolean | null;
  memberPresent: boolean | null;
  isInLupon: boolean | null;
...
export async function getRecordMinutesDetails(
  ...
  return apiFetch<RecordMinutesViewDTO>(
    `${LUPON_URL}/hearing-minutes-view/${hearingId}`,
  );
}
