...
export default function NewBlotterEntryPage() {
  const [complaintType, setComplaintType] = useState<"formal" | "record">("formal");
    ...
    <div className="min-h-screen ">
      ...
      <div className="px-3 sm:px-6 pt-4 sm:pt-5 pb-8 flex flex-col gap-4 sm:gap-5 max-w-6xl ml-4 mr-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <RadioCard name="complaint_type" value="formal" checked={complaintType === "formal"} onChange={() => setComplaintType("formal")} title="Formal Complaint" description="May Paanyaya at Salaysay – Ipapasa sa Lupon" />
          <RadioCard name="complaint_type" value="record" checked={complaintType === "record"} onChange={() => setComplaintType("record")} title="For the Record Only" description="Para sa record lang – Walang formal na aksyon" />
        </div>
        ...
        <SectionCard letter="B" title="Complainant Details">
          ...
          <FormInput label="House No. & Street" required placeholder="123 Rizal St." />
          <FormRow cols={3}>
