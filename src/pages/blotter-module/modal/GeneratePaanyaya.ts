...
export function generatePaanyaya(data: PaanyayaData): void {
  ...
  y += 5.5;
  doc.text(`Summon No.:  ${data.hearingNumber}`, pageW - MR, y, { align: "right" });
  ...
  // ══════════════════════════════════════════════════════════════════════════
  //  BODY TEXT
  //  - Font size 10pt (not 10.5) to avoid overflow
  //  - No em-space indent — jsPDF miscounts it and breaks splitTextToSize
  //  - No "justify" align — it stretches characters past the right margin
  // ══════════════════════════════════════════════════════════════════════════
  y += 12;
  doc.setFontSize(10);
  doc.setFont("times", "normal");
  ...
  doc.setFont("helvetica", "bold");
  doc.setTextColor(10, 10, 10);
  doc.text("MGA DETALYE NG MEDIASYON  /  HEARING DETAILS", cx, y, { align: "center" });
  ...
  y += 7;
  detailRow("Oras / Time:", `${fmtTime(data.startTime)} – ${fmtTime(data.endTime)}`, lCol);
  ...
  doc.setFont("helvetica", "normal");
  doc.setTextColor(25, 25, 25);
  // Venue text stays within box — limit to left column width only
