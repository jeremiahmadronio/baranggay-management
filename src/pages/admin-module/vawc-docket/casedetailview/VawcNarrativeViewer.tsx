...
import { FileText, Download, Loader2, BookOpen, File } from "lucide-react";
import { getVawcCaseNarrative } from "../../../../service/vawc-api/vawc-api";
...
// ─── Component ─────────────────────────────────────────────────────────────────
interface VawcNarrativeViewerProps {
  caseNumber: string;
}

export function VawcNarrativeViewer({ caseNumber }: VawcNarrativeViewerProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    getVawcCaseNarrative(caseNumber)
      ...
      <div className="relative bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        ...
        <div className="pl-5 pr-5 py-5">
          <p className="text-sm text-slate-800 leading-relaxed whitespace-pre-wrap font-[Georgia,serif]">
            ...
            {!expanded && hasMore && <span className="text-slate-400">…</span>}
          </p>
        </div>
