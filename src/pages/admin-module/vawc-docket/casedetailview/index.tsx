...
import { PermissionDeniedPage } from "../../blotter-docket/reusable/PermissionDeniedPage";
...
export default function CaseDetailsPage() {
  ...
  const id: number | string = rawId.startsWith("local_")
    ? rawId
    : Number(rawId);
  ...
  const [userAccess, setUserAccess] = useState<UserAccessPermission | null>(
    null,
  );
    ...
    try {
      ...
      setBpoActionMessage(
        err instanceof Error
          ? err.message
          : "Failed to generate BPO request letter.",
      );
    }
  };
  ...
  const handleWithdrawCase = async () => {
    if (!canResolveFinalize) {
      setWithdrawError(
        "You do not have permission to resolve and finalize cases.",
      );
      return;
    }
  ...
  if (error || !caseData) {
    return (
      <div className="min-h-screen">
        <div className="mx-auto px-6 py-6 space-y-5">
          <button
            onClick={() => navigate("/admin/vawc-cases")}
