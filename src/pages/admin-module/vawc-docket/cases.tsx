...
export default function AdminVawcCaseTable() {
  ...
  const columns: TableColumn<CaseSummaryDTO>[] = [
    { key: "caseNumber", header: "CASE NUMBER", align: "left" },
    {
      ...
      render: (item: CaseSummaryDTO) =>
        formatNameAsInitials(item.victimFullName),
    },
    {
      key: "violenceTypes",
      ...
      render: (item: CaseSummaryDTO) => (
        <button
          onClick={() => {
            ...
            navigate(`/admin/vawc-casedetailview?id=${item.id}`);
          }}
  ...
  const activeFilterCount = [violenceType, status, dateFrom, dateTo].filter(
    Boolean,
  ).length;
        ...
        <div className="mb-8">
          <KPIGrid columns={4}>
            <KPICard
              title="Pending Cases"
              value={
                loadingStats ? "—" : safeStats.totalPending.toLocaleString()
              }
              icon={KPIIcons["clock"]}
              color="amber"
            ...
            <KPICard
              ...
              value={
                loadingStats
                  ? "—"
                  : safeStats.totalExpiringSoon.toLocaleString()
              }
              icon={KPIIcons["month"]}
              color="rose"
