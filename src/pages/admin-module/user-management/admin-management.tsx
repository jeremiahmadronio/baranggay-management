...
import CreateAdminModal from "../../admin-root-module/create-admin-modal";
import CreateStaffModal from "./Create-Staff-Modal";
import { EditUserModal } from "../../admin-root-module/edit-user-modal";
import { EditStaffModal } from "./edit-staff-modal";
import { CreateAccountTypeModal } from "../../admin-root-module/create-account-type-modal";
...
const buildColumns = (
  ...
  },
  {
    ...
    header: "Account Type",
    render: (item) => {
      const rawType = String(item.accountType || "SYSTEM_USER")
        .trim()
        .toUpperCase();
      const type = rawType === "ADMIN" ? "SYSTEM_USER" : rawType;
      if (type === "SYSTEM_USER") {
        return (
...
export default function AdminUserManagement() {
  ...
  const fetchTable = useCallback(async () => {
    try {
      ...
      // Filter out archived admins and ADMIN account types
      const visibleTableData = content.filter((item) => {
        const status = (item.status || "").toUpperCase();
        const accountType = (item.accountType || "").trim().toUpperCase();
        return status !== "ARCHIVED" && accountType !== "ADMIN";
      });
      console.log("Table data fetched:", visibleTableData);
      setTableData(visibleTableData);

      // Use totalElements from API, or fallback to visibleTableData.length
        ...
        totalElementsFromApi > 0
          ? Math.max(
              ...
              totalElementsFromApi -
                visibleTableData.filter(
                  (item) =>
                    (item.accountType || "").trim().toUpperCase() === "ADMIN",
                ).length,
            )
