...
const recentActionsColumns: ColumnDef<RecentActions>[] = [
  ...
  {
    header: "Action",
    render: (row) => <span className="text-gray-500">{row.actionTaken}</span>,
  },
  {
    header: "Module",
    render: (row) => (
  ...
  },
  {
    ...
    render: (row) => (
      <span
        className={`text-xs font-semibold px-2.5 py-1 rounded-full ${getSeverityStyle(row.severity)}`}
      >
        {row.severity}
      </span>
    ),
        ...
        <div className="flex items-start justify-between mb-1">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">
              System usage across departments
            </p>
          </div>
