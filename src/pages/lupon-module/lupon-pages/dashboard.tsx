...
import {
  ...
  getRecentCases,
  getUpcomingHearings,
} from "../../../service/lupon-api/dashboard-api";
import type {
  DashboardStatsDTO,
  MonthlyCaseChartDTO,
...
type DistributionChartItem = CaseStatusDistributionDTO & { rawStatus: string };
...
function getDistributionColor(status: string, index: number): string {
  ...
  return (
    STATUS_DISTRIBUTION_COLORS[key] ?? DONUT_COLORS[index % DONUT_COLORS.length]
  );
}
...
function getStatusDistributionDescription(status: string): string {
  ...
  const map: Record<string, string> = {
    ...
    DISMISSED: "Dismissed and closed",
    CERTIFIED_TO_FILE_ACTION: "Certified for formal filing",
    EXPIRED_UNACTIONED: "No action before deadline",
...
const LupongTagapamayapaDashboard = () => {
  ...
  const [chartData, setChartData] = useState<MonthlyCaseChartDTO[]>([]);
  const [distribution, setDistribution] = useState<DistributionChartItem[]>([]);
  const [recentCases, setRecentCases] = useState<RecentCaseDTO[]>([]);
  const [hearings, setHearings] = useState<UpcomingHearingDTO[]>([]);
                    ...
                    <PieChart>
                      ...
                      >
                        {distribution.map((item, index) => (
                          <Cell
                            key={`dist-${index}`}
                            fill={getDistributionColor(item.rawStatus, index)}
                          />
                        ))}
                      </Pie>
                      <Tooltip
                      ...
                      <div className="flex items-start gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500 leading-tight mt-0.5">
                            {getStatusDistributionDescription(item.rawStatus)}
                          </p>
                        </div>
                      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
          ...
          <div className="lg:col-span-4 bg-white rounded-lg border border-gray-200 flex flex-col overflow-hidden">
            <div className="p-5 border-b border-gray-200">
              ...
              <p className="text-sm text-gray-500 mt-1">
                Next scheduled conciliation sessions
              </p>
            </div>
            <div className="p-5 space-y-5 flex-1">
              {loading ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-400 space-y-2">
                  <NoRecords text="No upcoming conciliations." />
                </div>
              )}
            </div>
          </div>
...
export default LupongTagapamayapaDashboard;
