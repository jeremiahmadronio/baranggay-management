...
import {
  mockLuponCasesTrend,
  mockLuponReportStats,
  mockLuponStatusStats,
  mockLuponTopNature,
...
export function KapitanaLuponReportsPage() {
  ...
  const fetchDashboardData = async (start: string, end: string) => {
    ...
    try {
      if (useKapitanaMockData()) {
        setStats(mockLuponReportStats());
        const statusRes = mockLuponStatusStats();
        const natureRes = mockLuponTopNature();
        const trendRes = mockLuponCasesTrend();
