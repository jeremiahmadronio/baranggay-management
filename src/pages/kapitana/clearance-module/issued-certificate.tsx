...
import {
  mockKapitanaIssuedCertificateList,
  mockKapitanaIssuedStats,
...
export const KapitanaIssuedCertificatePage = () => {
  ...
  const loadTable = useCallback(async (): Promise<IssuedCertificate[]> => {
    try {
      ...
      if (useKapitanaMockData()) {
        baseRows = mockKapitanaIssuedCertificateList();
        ...
        if (dateFrom) {
          activeRows = activeRows.filter((c) =>
            isInDateRange(c.dateIssued, dateFrom, dateTo),
          );
        }
        if (dateTo) {
          activeRows = activeRows.filter((c) =>
  ...
  useEffect(() => {
    const loadAll = async () => {
      ...
      try {
        if (useKapitanaMockData()) {
          setKPIData(mockKapitanaIssuedStats());
          await loadTable();
          return;
        }
...
export default KapitanaIssuedCertificatePage;
