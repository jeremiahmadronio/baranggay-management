...
export function BlotterDocketDetailView({
  ...
  useEffect(() => {
    if (useKapitanaMockData()) {
      ...
      setCanManageMediation(true);
      setCanManageLuponEscalation(true);
      setCanResolveFinalize(true);
      setCanManageCaseNotes(true);
      setCanUpdateCaseInfo(true);
    ...
    getMyAccess()
      .then((access) => {
        setCanViewCases(
          hasBlotterPermission(access, BLOTTER_PERMISSIONS.VIEW_CASES),
        );
        setCanManageMediation(
          hasBlotterPermission(access, BLOTTER_PERMISSIONS.MANAGE_MEDIATION),
        );
        setCanManageLuponEscalation(
          hasBlotterPermission(
            access,
            BLOTTER_PERMISSIONS.MANAGE_LUPON_ESCALATION,
          ),
        );
        setCanResolveFinalize(
          hasBlotterPermission(
            access,
            BLOTTER_PERMISSIONS.RESOLVE_FINALIZE_CASE,
          ),
        );
        setCanManageCaseNotes(
          hasBlotterPermission(access, BLOTTER_PERMISSIONS.MANAGE_CASE_NOTES),
        );
        setCanUpdateCaseInfo(
          hasBlotterPermission(
            access,
            BLOTTER_PERMISSIONS.UPDATE_CASE_INFORMATION,
          ),
        );
      })
      .catch(() => {
        setCanViewCases(false);
        setCanManageMediation(false);
        setCanManageLuponEscalation(false);
        setCanResolveFinalize(false);
        setCanManageCaseNotes(false);
        setCanUpdateCaseInfo(false);
      })
      .finally(() => {
        setAccessLoading(false);
      });
  ...
  const handleUpdateStatus = async (
    ...
    try {
      if (useKapitanaMockData()) {
        setModal(null);
        setDocket((prev) =>
          prev ? { ...prev, caseStatus: statusToSend } : prev,
        );
        return;
      }
  ...
  const handleReferConfirm = async (members: any[]) => {
    ...
    try {
      if (useKapitanaMockData()) {
        ...
        setDocket((prev) =>
          prev ? { ...prev, caseStatus: "REFERRED_TO_LUPON" } : prev,
        );
        return;
      }
