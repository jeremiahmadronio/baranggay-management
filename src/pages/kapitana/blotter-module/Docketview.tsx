...
const KapitanaDocketview = () => {
  ...
  const fetchTable = useCallback(async (p: DocketTableParams) => {
    ...
    try {
      if (useKapitanaMockData()) {
        const data = mockDocketTablePage(p);
  ...
  useEffect(() => {
    if (useKapitanaMockData()) {
      setNatureOptions(mockKapitanaNatureOptions);
      Promise.all([fetchStats(), fetchTable(params)]);
      return;
    }
    Promise.all([
      fetchStats(),
  ...
  // ── Permissions ───────────────────────────────────────────────────────────
  // All permissions granted for Kapitana/Captain
  const canView = true;
...
export default KapitanaDocketview;
