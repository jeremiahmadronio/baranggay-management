...
import type { CaseTimeLineDTO } from '../../../../service/vawc-api/vawc-api';
...
const toDateAndTime = (raw: string) => {
  ...
  return {
    date: parsed.toLocaleDateString('en-PH', { year: 'numeric', month: 'short', day: 'numeric' }),
...
export function TimelineTab({ timeline, timelineLoading }: TimelineTabProps) {
  return (
    <SectionCard title="Case History & Timeline" icon={<ClockIcon className="w-4 h-4 text-gray-400" />}>
      {timelineLoading ? (
