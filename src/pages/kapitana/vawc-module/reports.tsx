...
import { getKapitanaVawcMockReportPayload } from "./mock-data";
import {
  CenteredLoader,
  CircleLoader,
  NoRecords,
  ...
  const fetchAll = useCallback(async (start: string, end: string) => {
    ...
    try {
      if (useKapitanaMockData()) {
        const m = getKapitanaVawcMockReportPayload(start, end);
        setStats(m.stats);
        setTrend(m.trend);
        setNature(m.nature);
      ...
      const [statsRes, natureRes, trendRes, categoryRes] = await Promise.all([
