import { FileTextIcon, ShieldAlertIcon, ClipboardPenIcon, FileOutputIcon, XCircleIcon, UserIcon } from 'lucide-react';
...
export function OverviewTab({
  ...
  const elapsedDays = startDate
    ...
    : 0;
  const progressPercent = Math.min(100, Math.max(0, (elapsedDays / totalDurationDays) * 100));
  ...
  return (
    <div className="space-y-5">
      <SectionCard title="15-Day Duration" icon={<FileTextIcon className="w-4 h-4 text-gray-400" />}>
        <div className="space-y-3">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-medium text-gray-800">15-Day Monitoring Period</p>
              <p className="mt-1 text-xs text-gray-500">Track the case duration based on the filed date and current deadline.</p>
            </div>
            <span className="text-sm font-medium text-blue-600">{remainingText}</span>
          </div>
      ...
      <div>
        ...
        {isWithdrawn && (
          <p className="mb-3 text-xs font-medium text-gray-500">This case is withdrawn and can no longer be changed.</p>
        )}
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <button
            ...
            className={`flex flex-col items-start gap-2 p-5 border shadow-sm rounded-xl text-left focus:outline-none ${
              isWithdrawn ? 'bg-gray-50 border-gray-200 opacity-60 cursor-not-allowed' : 'bg-white border-gray-200'
            }`}
          >
            <div className={`p-2.5 rounded-lg ${isWithdrawn ? 'bg-gray-100' : 'bg-blue-50'}`}>
              <ShieldAlertIcon className={`w-5 h-5 ${isWithdrawn ? 'text-gray-400' : 'text-blue-600'}`} />
            </div>
            <span className={`text-sm ${isWithdrawn ? 'text-gray-500' : 'text-blue-600'}`}>Issue BPO</span>
          ...
          <button
            ...
            title={isWithdrawn ? 'Withdrawn cases are read-only.' : canRecordIntervention ? 'Open BPO intervention logs' : 'Activate the BPO first before recording an intervention.'}
            className={`flex flex-col items-start gap-2 p-5 border shadow-sm rounded-xl text-left focus:outline-none ${
          ...
          >
            <div className={`p-2.5 rounded-lg ${!isWithdrawn && canRecordIntervention ? 'bg-emerald-50' : 'bg-gray-100'}`}>
              <ClipboardPenIcon className={`w-5 h-5 ${!isWithdrawn && canRecordIntervention ? 'text-emerald-600' : 'text-gray-400'}`} />
            </div>
            <span className={`text-sm ${!isWithdrawn && canRecordIntervention ? 'text-emerald-600' : 'text-gray-500'}`}>Record Intervention</span>
            <span className="text-xs text-gray-500">
              {isWithdrawn ? 'Case is read-only' : canRecordIntervention ? 'Log activity or visit' : 'Activate BPO first'}
            </span>
          </button>
          ...
          >
            <div className="p-2.5 rounded-lg bg-gray-100">
              <FileOutputIcon className={`w-5 h-5 ${isWithdrawn ? 'text-gray-400' : 'text-gray-600'}`} />
            </div>
            <span className={`text-sm ${isWithdrawn ? 'text-gray-500' : 'text-gray-700'}`}>Referral Letter</span>
            <span className="text-xs text-gray-500">{isWithdrawn ? 'Case is read-only' : 'Export case summary'}</span>
          </button>
          ...
          >
            <div className={`p-2.5 rounded-lg ${isWithdrawn ? 'bg-gray-100' : 'bg-rose-50'}`}>
              <XCircleIcon className={`w-5 h-5 ${isWithdrawn ? 'text-gray-400' : 'text-rose-600'}`} />
            </div>
            <span className={`text-sm ${isWithdrawn ? 'text-gray-500' : 'text-rose-600'}`}>Withdraw</span>
            <span className="text-xs text-gray-500">{isWithdrawn ? 'Case already withdrawn' : 'Withdraw this case'}</span>
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          <SectionCard title="Victim Information" icon={<UserIcon className="w-4 h-4 text-gray-400" />}>
            <div className="grid grid-cols-2 gap-4">
              <InfoField label="Full Name" value={victimFullName} />
              <InfoField label="Contact Number" value={caseData.contactNumber} />
              <InfoField label="Age" value={caseData.age ? `${caseData.age} years old` : undefined} />
              ...
              <div className="col-span-2">
                <InfoField label="Complete Address" value={caseData.completeAddress} />
              </div>
            </div>
          </SectionCard>

          <SectionCard title="Respondent Information" icon={<UserIcon className="w-4 h-4 text-gray-400" />}>
            <div className="grid grid-cols-2 gap-4">
              ...
              <InfoField label="Civil Status" value={caseData.respondentCivilStatus} />
              <InfoField label="Relationship to Complainant" value={caseData.relationshipToComplainant} />
              <div>
                <p className="text-xs font-medium text-gray-600 uppercase tracking-wider mb-1">Living with Complainant</p>
                <span className={`text-xs px-2.5 py-1 rounded-full ${caseData.livingWithComplainant ? 'bg-amber-50 text-amber-700' : 'bg-gray-100 text-gray-600'}`}>
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6 space-y-5 h-fit">
          ...
          <div>
            ...
            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-normal ${pillClass}`}>
              {caseStatus ? (STATUS_LABEL_OVERRIDES[caseStatus] ?? caseStatus.replace(/_/g, ' ')) : 'UNKNOWN'}
            </span>
          </div>
          ...
          {caseData.evidenceNames && caseData.evidenceNames.length > 0 && (
            <div>
              <p className="text-xs font-medium text-gray-600 uppercase tracking-wider mb-1.5">Evidence Submitted</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" role="dialog" aria-modal="true">
          <div className="w-full max-w-lg rounded-xl bg-white shadow-xl">
            <div className="border-b border-gray-200 px-6 py-4">
              ...
              <p className="mt-1 text-sm text-gray-500">Provide the reason for withdrawing this VAWC case.</p>
            </div>
            <div className="space-y-3 px-6 py-5">
              ...
              {withdrawMessage && <p className="text-xs text-emerald-600">{withdrawMessage}</p>}
            </div>
      ...
      <SectionCard title="Incident Details" icon={<FileTextIcon className="w-4 h-4 text-gray-400" />}>
        <p className="text-sm text-gray-900 leading-relaxed">
          {caseData.narrative || '\u2014'}
        </p>
        {caseData.descriptionOfInjuries && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <p className="text-xs font-medium text-gray-600 uppercase tracking-wider mb-1.5">Description of Injuries</p>
            <p className="text-sm text-gray-900">{caseData.descriptionOfInjuries}</p>
          </div>
        )}
      </SectionCard>
