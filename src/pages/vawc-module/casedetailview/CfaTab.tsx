...
function buildVictimSentence(displayCfa: DisplayCFADTO, caseData: CaseViewDTO): string {
  const age = caseData.age ? `, ${caseData.age} years old,` : ',';
  const address = displayCfa.complainantAddress || caseData.completeAddress || 'address not available';
  return `This is to formally refer to your good office the case of ${displayCfa.complainantName}${age} residing at ${address}.`;
}

function buildIncidentSentence(displayCfa: DisplayCFADTO, caseData: CaseViewDTO): string {
  ...
  const relationship = caseData.relationshipToComplainant
    ...
    : '';
  const nature = (displayCfa.matterFiled || caseData.natureOfComplaint || 'VAWC incident').toLowerCase();
...
function buildNarrativeBlock(displayCfa: DisplayCFADTO, caseData: CaseViewDTO): string {
...
function buildFallbackCfaDetails(
  ...
  const complainantName = [caseData.firstName, caseData.middleName, caseData.lastName]
    .filter(Boolean)
    .join(' ') || 'Complainant';
...
export function CfaTab({
  ...
  const isCertifiedToFileAction = (caseData.caseStatus || '').toUpperCase() === 'CERTIFIED_TO_FILE_ACTION';
  ...
  const canEditSubject = OTHERS_KEYWORDS.some((keyword) => (caseData.natureOfComplaint || '').toLowerCase().includes(keyword));

  const canSubmit = referralForm.grounds.trim().length > 0 && referralForm.subjectOfLitigation.trim().length > 0 && !referralLoading;

  const wait = (ms: number) => new Promise((resolve) => window.setTimeout(resolve, ms));

  const loadCfaDetails = async (showLoader = true): Promise<DisplayCFADTO | null> => {
    try {
      if (showLoader) setCfaLoading(true);
      setCfaError('');
      const data = await getVawcCfa(caseId, caseData.caseNumber);
      setCfaDetails(data);
      return data;
    } catch (err) {
      console.error('Failed to load CFA details:', err);
      setCfaDetails(null);
      const message = err instanceof Error ? err.message : 'Failed to load CFA details.';
  ...
  useEffect(() => {
    setReferralForm((current) => ({
      ...current,
      subjectOfLitigation: current.subjectOfLitigation || caseData.natureOfComplaint || '',
    }));
  }, [caseData.natureOfComplaint]);

  useEffect(() => {
    void loadCfaDetails(true);
  }, [caseId, caseData.caseStatus]);
  ...
  const handleReferralFormChange = (field: 'grounds' | 'subjectOfLitigation', value: string) => {
    setReferralForm((current) => ({ ...current, [field]: value }));
  };

  const handleCreateReferral = async () => {
    if (!referralForm.grounds.trim() || !referralForm.subjectOfLitigation.trim()) {
    ...
    try {
      ...
      await createReferral({
        caseId,
        blotterNumber: caseData.caseNumber,
        grounds: referralForm.grounds.trim(),
        subjectOfLitigation: referralForm.subjectOfLitigation.trim(),
        matterFiled: referralForm.subjectOfLitigation.trim(),
      });
      const issuedCfa = await fetchIssuedCfaForDownload();
      const resolvedCfa = issuedCfa ?? buildFallbackCfaDetails(caseData, caseId, {
        matterFiled: referralForm.subjectOfLitigation.trim(),
        grounds: referralForm.grounds.trim(),
      });

      if (!issuedCfa) {
        setReferralMessage('Referral created successfully. The saved CFA document is still syncing, so a case-based Word copy is being downloaded.');
      } else {
        setReferralMessage('Referral created successfully. The Word file is downloading.');
      }

      setCfaDetails(resolvedCfa);
      downloadVawcCFAasWord(resolvedCfa, caseData);
      setReferralForm((current) => ({
        ...current,
        grounds: '',
      }));
  ...
  return (
    <SectionCard title="Referral / CFA" icon={<FileTextIcon className="w-4 h-4 text-gray-400" />}>
      {cfaLoading ? (
        ...
        <div className="flex flex-col items-center justify-center gap-3 py-24">
          ...
          <div className="text-center">
            <p className="text-sm font-semibold text-gray-800">
              {isCertifiedToFileAction ? 'CFA has been issued, but the document could not be loaded.' : 'Unable to load CFA details'}
            </p>
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 rounded-lg px-3 py-2 text-emerald-700">
              {fallbackCertifiedCfa && !cfaDetails ? (
                <span className="text-xs font-medium">Preview generated from case record</span>
              ) : <span className="text-xs font-medium"></span>}
            </div>
            <div className="flex gap-2">
              <button
                onClick={() => void downloadVawcCFAasWord(displayCfa, caseData)}
          ...
          <div className="rounded-2xl bg-white p-8 shadow-sm ring-1 ring-slate-200">
            <div className="mx-auto max-w-3xl bg-white px-14 py-12 text-slate-900" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
              <div className="text-center">
                <p className="text-[13px] font-bold uppercase tracking-wide">Republic of the Philippines</p>
                <h2 className="mt-1 text-[19px] font-bold">Barangay VAWC Desk</h2>
                <p className="text-[12px] text-slate-700">Valenzuela City</p>
                <p className="text-[12px] text-slate-500">Province, Municipality</p>
              </div>
              ...
              <div className="mt-10 space-y-6 text-[14px] leading-8 text-slate-800">
                ...
                <div>
                  ...
                  <p className="whitespace-pre-wrap">{buildNarrativeBlock(displayCfa, caseData)}</p>
                </div>
                <p>
                  We are referring this case for appropriate counseling, psychological support, and
                  further intervention as deemed necessary by your office.
                </p>
              </div>
              ...
              <div className="mt-20 text-[14px] leading-7">
                ...
                <div className="mt-12">
                  <p className="font-bold uppercase">Officer {displayCfa.assignOfficerName}</p>
        ...
        <div className="space-y-5">
          <div className="rounded-xl border border-blue-200 bg-blue-50 px-6 py-8 text-sm text-blue-900">
            <p className="font-semibold">This case is already marked as Certified To File Action.</p>
            <p className="mt-2 text-blue-800">
              The CFA document is being loaded from the saved case record. If it does not appear yet, retry loading the document below.
            </p>
        ...
        <div className="space-y-5">
          <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            <div className="flex items-center gap-2.5 border-b border-gray-100 px-6 py-4">
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-900">Issue Referral / CFA</h3>
                <p className="text-xs text-gray-500">Case No. {caseData.caseNumber}</p>
              </div>
            </div>
            ...
            <div className="space-y-5 p-6">
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold uppercase tracking-wider text-gray-600">
                  Subject of Litigation <span className="text-red-500">*</span>
                </label>
                {canEditSubject ? (
                  <input
                    type="text"
                    value={referralForm.subjectOfLitigation}
                    onChange={(event) => handleReferralFormChange('subjectOfLitigation', event.target.value)}
                  ...
                  <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm font-medium text-gray-800">
                    {referralForm.subjectOfLitigation || caseData.natureOfComplaint || '—'}
                  </div>
                )}
                {!canEditSubject && (
                  <p className="text-xs text-gray-400">This field can only be edited when the complaint nature is marked as Others.</p>
                )}
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  onChange={(event) => handleReferralFormChange('grounds', event.target.value)}
                  rows={5}
                ...
                />
                <p className="text-xs text-gray-400">Ilagay ang dahilan kung bakit maaari nang i-refer o iakyat ang kasong ito.</p>
              </div>
