...
function generateCaseNumber(): string {
  ...
  return `${year}-VAWC-${Math.floor(Math.random() * 10000).toString().padStart(4, '0')}`
}
...
function hasWitnessContent(witness: WitnessEntry): boolean {
  return Boolean(
    witness.fullName.trim() ||
    witness.contactNumber.trim() ||
    witness.address.trim() ||
    witness.testimony.trim(),
  )
}
...
const SectionCard = ({
  ...
  <BlotterSectionCard letter={step} title={title} notice={notice} noticeTone={noticeTone}>
    {children}
  </BlotterSectionCard>
)
...
const CharCounter = ({ current, max }: { current: number; max: number }) => (
  <div className="flex justify-end pr-1 mt-1">
    <span className={`text-[10px] ${current >= max ? 'text-red-500 font-bold' : 'text-slate-400'}`}>
      {current}/{max}
    </span>
  </div>
)
...
const emptyWitness = (): WitnessEntry => ({ fullName: '', contactNumber: '', address: '', testimony: '' })

const splitWitnessName = (fullName: string): { firstName: string; lastName: string } => {
...
const sanitizeComplaintForm = (
  complainant: {
    lastName: string; firstName: string; middleName: string; contact: string; age: string; gender: string; civilStatus: string; email: string; address: string
  },
  respondent: {
    lastName: string; firstName: string; middleName: string; alias: string; contact: string; age: string; dob: string; gender: string; civilStatus: string; address: string
  },
  relationshipTypeName: string,
  incident: {
    dateOfIncident: string; timeOfIncident: string; placeOfIncident: string; frequencyOfIncident: string; descriptionOfInjuries: string
  },
  narrative: string,
  witnesses: WitnessEntry[],
  ...
  incident: {
    ...
    frequencyOfIncident: sanitizeText(incident.frequencyOfIncident, LIMITS.shortTextMax),
    descriptionOfInjuries: sanitizeText(incident.descriptionOfInjuries, LIMITS.descriptionMax),
  },
  narrative: sanitizeParagraph(narrative, LIMITS.narrativeMax),
  witnesses: witnesses.map((witness) => ({
    fullName: sanitizeText(witness.fullName, LIMITS.nameMax),
...
export function VAWCNewComplaint() {
  ...
  const [officerOptions, setOfficerOptions] = useState<AssignOfficerOptionDTO[]>([])
  ...
  const [selectedRespondentId, setSelectedRespondentId] = useState<number | undefined>()
  ...
  const [complainant, setComplainant] = useState({
    ...
    contact: '', age: '', gender: '', civilStatus: '',
    email: '', address: '',
  })
  ...
  const [respondent, setRespondent] = useState({
    ...
    contact: '', age: '', dob: '', gender: '', civilStatus: '',
    address: '',
  })
  ...
  const [incident, setIncident] = useState({
    ...
    placeOfIncident: '', frequencyOfIncident: '',
  ...
  const persistComplaintDraft = () => {
    sessionStorage.setItem(COMPLAINT_DRAFT_KEY, JSON.stringify(buildComplaintDraft()))
  }
  ...
  useEffect(() => {
    const loadOptions = async () => {
      try {
        const [officersResult, violenceResult, evidenceResult] = await Promise.allSettled([
          getAssignOfficerComplaintOptions(),
          getViolenceOptions(),
          getEvidenceOptions(),
        ])
        const warnings: string[] = []
        if (officersResult.status === 'fulfilled') setOfficerOptions(officersResult.value)
        else { setOfficerOptions([]); warnings.push('reporting officers') }
        if (violenceResult.status === 'fulfilled') setViolenceOptions(violenceResult.value)
        else { setViolenceOptions([]); warnings.push('violence options') }
        if (evidenceResult.status === 'fulfilled') setEvidenceOptions(evidenceResult.value)
        else setEvidenceOptions([])
        setOptionsWarning(warnings.length ? `Some dropdown data could not be loaded: ${warnings.join(', ')}.` : null)
      } finally {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setSelectedViolenceIds(Array.isArray(draft.selectedViolenceIds) ? draft.selectedViolenceIds : [])
      setSelectedEvidenceIds(Array.isArray(draft.selectedEvidenceIds) ? draft.selectedEvidenceIds : [])
      ...
      setWitnesses(Array.isArray(draft.witnesses) && draft.witnesses.length > 0 ? draft.witnesses : [emptyWitness()])
    } catch {
      sessionStorage.removeItem(COMPLAINT_DRAFT_KEY)
    }
  }, [])
  ...
  const toggleViolence = (id: number) =>
    setSelectedViolenceIds((prev) => (prev.includes(id) ? prev.filter((v) => v !== id) : [...prev, id]))
  const toggleEvidence = (id: string) =>
    setSelectedEvidenceIds((prev) => (prev.includes(id) ? prev.filter((v) => v !== id) : [...prev, id]))
  ...
  const applyComplainant = (person: PersonSearchResponseDTO) => {
    ...
    ;['cLastName', 'cFirstName', 'cContact', 'cAge', 'cGender', 'cCivilStatus', 'cAddress'].forEach(clearErr)
  }
  ...
  const buildComplaintFingerprint = (dto: ComplaintDTO) =>
    JSON.stringify({
      ...
      complainantFirstName: dto.complainantFirstName?.trim().toLowerCase() ?? '',
      ...
      relationshipTypeName: dto.relationshipTypeName?.trim().toLowerCase() ?? '',
  ...
  const hasRecentMatchingSubmission = (fingerprint: string) => {
    ...
    if (!raw) return false
    try {
      const parsed = JSON.parse(raw) as { fingerprint?: string; submittedAt?: number }
      if (parsed.fingerprint === fingerprint && typeof parsed.submittedAt === 'number' && Date.now() - parsed.submittedAt < RECENT_SUBMISSION_WINDOW_MS) return true
    } catch { sessionStorage.removeItem(RECENT_SUBMISSION_KEY) }
    return false
  }
  ...
  const validate = (): boolean => {
    ...
    else if (sanitized.complainant.lastName.length < LIMITS.nameMin) e.cLastName = 'Last name must be at least 2 characters.'
    else if (!isValidName(sanitized.complainant.lastName)) e.cLastName = 'Last name can only contain letters, spaces, apostrophes, periods, commas, and hyphens.'

    if (!sanitized.complainant.firstName) e.cFirstName = 'First name is required.'
    else if (sanitized.complainant.firstName.length < LIMITS.nameMin) e.cFirstName = 'First name must be at least 2 characters.'
    else if (!isValidName(sanitized.complainant.firstName)) e.cFirstName = 'First name can only contain letters, spaces, apostrophes, periods, commas, and hyphens.'

    if (sanitized.complainant.middleName && !isValidName(sanitized.complainant.middleName)) e.cMiddleName = 'Middle name contains invalid characters.'
    if (sanitized.complainant.contact && !isValidPhone(sanitized.complainant.contact)) e.cContact = 'Contact number must start with 09 and contain 11 digits.'
    if (!sanitized.complainant.age) e.cAge = 'Age is required.'
    else {
      const complainantAge = Number(sanitized.complainant.age)
      if (Number.isNaN(complainantAge) || complainantAge < 1 || complainantAge > 130) e.cAge = 'Age must be between 1 and 130.'
    }
    if (!sanitized.complainant.gender) e.cGender = 'Gender is required.'
    if (sanitized.complainant.email && !isValidEmail(sanitized.complainant.email)) e.cEmail = 'Email address must be in a valid format.'
    if (!sanitized.complainant.address) e.cAddress = 'Complete address is required.'
    else if (sanitized.complainant.address.length < LIMITS.addressMin) e.cAddress = 'Complete address must be at least 8 characters.'

    if (!sanitized.respondent.lastName) e.rLastName = 'Last name is required.'
    else if (sanitized.respondent.lastName.length < LIMITS.nameMin) e.rLastName = 'Last name must be at least 2 characters.'
    else if (!isValidName(sanitized.respondent.lastName)) e.rLastName = 'Last name can only contain letters, spaces, apostrophes, periods, commas, and hyphens.'

    if (!sanitized.respondent.firstName) e.rFirstName = 'First name is required.'
    else if (sanitized.respondent.firstName.length < LIMITS.nameMin) e.rFirstName = 'First name must be at least 2 characters.'
    else if (!isValidName(sanitized.respondent.firstName)) e.rFirstName = 'First name can only contain letters, spaces, apostrophes, periods, commas, and hyphens.'

    if (sanitized.respondent.middleName && !isValidName(sanitized.respondent.middleName)) e.rMiddleName = 'Middle name contains invalid characters.'
    if (sanitized.respondent.alias && !isValidName(sanitized.respondent.alias)) e.rAlias = 'Alias contains invalid characters.'
    if (sanitized.respondent.contact && !isValidPhone(sanitized.respondent.contact)) e.rContact = 'Contact number must start with 09 and contain 11 digits.'
    if (sanitized.respondent.age) {
      const respondentAge = Number(sanitized.respondent.age)
      if (Number.isNaN(respondentAge) || respondentAge < 1 || respondentAge > 130) e.rAge = 'Age must be between 1 and 130.'
    }
    if (sanitized.respondent.address && sanitized.respondent.address.length < LIMITS.addressMin) e.rAddress = 'Complete address must be at least 8 characters.'
    if (!sanitized.relationshipTypeName) e.rRelationship = 'Relationship to victim is required.'
    if (!natureOfComplaintId) e.natureOfComplaintId = 'Nature of complaint is required.'
    if (!sanitized.incident.dateOfIncident) e.dateOfIncident = 'Date of incident is required.'
    if (!sanitized.incident.placeOfIncident) e.placeOfIncident = 'Place of incident is required.'
    else if (sanitized.incident.placeOfIncident.length < LIMITS.locationMin) e.placeOfIncident = 'Place of incident must be at least 3 characters.'
    if (selectedViolenceIds.length === 0) e.violence = 'Select at least one type of violence.'
    if (!sanitized.narrative) e.narrative = 'Narrative statement is required.'
    else if (sanitized.narrative.length < LIMITS.narrativeMin) e.narrative = 'Narrative statement must be at least 20 characters.'

    sanitized.witnesses.forEach((witness, index) => {
      if (!hasWitnessContent(witness)) return
      const key = `witness-${index}`
      if (!witness.fullName) e[key] = `Witness ${index + 1}: full name is required.`
      else if (witness.fullName.length < LIMITS.nameMin) e[key] = `Witness ${index + 1}: full name must be at least 2 characters.`
      else if (!isValidName(witness.fullName)) e[key] = `Witness ${index + 1}: full name contains invalid characters.`
      else if (witness.contactNumber && !isValidPhone(witness.contactNumber)) e[key] = `Witness ${index + 1}: contact number must start with 09 and contain 11 digits.`
      else if (witness.address && witness.address.length < LIMITS.addressMin) e[key] = `Witness ${index + 1}: address must be at least 8 characters.`
    })

    if (!certified) e.certified = 'You must certify the information is true and correct.'
    setErrors(e)
  ...
  const buildDTO = (): ComplaintDTO => {
    ...
    return ({
    // complainant: if selected from search, send ID only; otherwise send manual fields
    ...(selectedComplainantId
      ...
      : {
          ...
          complainantAge: sanitized.complainant.age ? Number(sanitized.complainant.age) : undefined,
    ...
    // respondent: if selected from search, send ID only; otherwise send manual fields
    ...(selectedRespondentId
      ...
      : {
          ...
          respondentAge: sanitized.respondent.age ? Number(sanitized.respondent.age) : undefined,
    ...
    timeOfIncident: sanitized.incident.timeOfIncident ? `${sanitized.incident.timeOfIncident}:00` : undefined,
    placeOfIncident: sanitized.incident.placeOfIncident,
    frequencyOfIncident: sanitized.incident.frequencyOfIncident || undefined,
    descriptionOfInjuries: sanitized.incident.descriptionOfInjuries || undefined,
    ...
    evidenceTypeIds: selectedEvidenceIds.length ? selectedEvidenceIds : undefined,
    witnesses: sanitized.witnesses.filter((w) => w.fullName).map((w) => {
      const parsedName = splitWitnessName(w.fullName)
      return {
  ...
  const handleConfirmedSubmit = async () => {
    ...
    try {
      ...
      sessionStorage.setItem(RECENT_SUBMISSION_KEY, JSON.stringify({ fingerprint, submittedAt: Date.now() }))
      clearComplaintDraft()
  ...
  return (
    <div className="min-h-screen bg-blue-50/40">
      <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col gap-6">
        ...
        <ActionModal isOpen={showSuccessModal} onClose={handleSuccessClose} title="Case Filed!" type="success">
        ...
        <SectionCard step="A" title="Case Information">
          <div className="bg-blue-50/40 border border-blue-100 rounded-lg px-5 py-4 shadow-sm grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              { label: 'Reporting Officer', value: officerName, hint: '(logged in)' },
            ].map((f) => (
              <div key={f.label} className="text-center">
                <p className="text-xs font-semibold text-slate-600 tracking-wide mb-1">{f.label}</p>
          ...
          <FormRow cols={2}>
            <FormSelect
              ...
              value={assignToId}
              onChange={(e) => { setAssignToId(e.target.value); clearErr('assignToId') }}
        ...
        <SectionCard step="B" title="Complainant Information">
          <PersonSearchField label="Search Complainant (Auto-fill)" placeholder="Search by name..." onSelect={applyComplainant} />

          <FormRow cols={3}>
            <FormInput id="field-cLastName" label="Last Name" required placeholder="e.g. Dela Cruz" value={complainant.lastName} onChange={(e) => { updateComplainant('lastName', e.target.value); clearErr('cLastName') }} error={errors.cLastName} />
            <FormInput id="field-cFirstName" label="First Name" required placeholder="e.g. Maria" value={complainant.firstName} onChange={(e) => { updateComplainant('firstName', e.target.value); clearErr('cFirstName') }} error={errors.cFirstName} />
            <FormInput id="field-cMiddleName" label="Middle Name" placeholder="e.g. Santos" value={complainant.middleName} onChange={(e) => updateComplainant('middleName', e.target.value)} />
          </FormRow>

          <FormRow cols={3}>
            <FormInput id="field-cContact" label="Contact Number" placeholder="09XX XXX XXXX" inputMode="numeric" maxLength={11} value={complainant.contact} onChange={(e) => { updateComplainant('contact', e.target.value.replace(/\D/g, '')); clearErr('cContact') }} error={errors.cContact} />
            <FormInput id="field-cAge" label="Age" required placeholder="e.g. 35" inputMode="numeric" maxLength={3} value={complainant.age} onChange={(e) => { updateComplainant('age', e.target.value.replace(/\D/g, '').slice(0, 3)); clearErr('cAge') }} error={errors.cAge} />
            <FormSelect id="field-cGender" label="Gender" required options={GENDER_OPTIONS} placeholder="Select Gender" value={complainant.gender} onChange={(e) => { updateComplainant('gender', e.target.value); clearErr('cGender') }} error={errors.cGender} />
          </FormRow>

          <FormRow cols={2}>
            <FormSelect label="Civil Status" options={CIVIL_STATUS_OPTIONS} placeholder="Select Civil Status" value={complainant.civilStatus} onChange={(e) => updateComplainant('civilStatus', e.target.value)} />
            <FormInput id="field-cEmail" label="Email Address" type="email" placeholder="email@example.com" maxLength={50} value={complainant.email} onChange={(e) => updateComplainant('email', e.target.value)} />
          </FormRow>

          <FormInput id="field-cAddress" label="Complete Address" required placeholder="House No., Street, Barangay, Municipality/City" value={complainant.address} onChange={(e) => { updateComplainant('address', e.target.value); clearErr('cAddress') }} error={errors.cAddress} />
        </SectionCard>
        ...
        <SectionCard
          ...
          noticeTone="warning"
        >
          <PersonSearchField label="Search Respondent (Auto-fill)" placeholder="Search by name..." onSelect={applyRespondent} />

          <FormRow cols={3}>
            <FormInput id="field-rLastName" label="Last Name" required placeholder="e.g. Santos" value={respondent.lastName} onChange={(e) => { updateRespondent('lastName', e.target.value); clearErr('rLastName') }} error={errors.rLastName} />
            <FormInput id="field-rFirstName" label="First Name" required placeholder="e.g. Pedro" value={respondent.firstName} onChange={(e) => { updateRespondent('firstName', e.target.value); clearErr('rFirstName') }} error={errors.rFirstName} />
            <FormInput id="field-rMiddleName" label="Middle Name" placeholder="e.g. Reyes" value={respondent.middleName} onChange={(e) => updateRespondent('middleName', e.target.value)} />
          </FormRow>

          <FormRow cols={4}>
            <FormInput id="field-rAlias" label="Alias / Nickname" placeholder="If any" value={respondent.alias} onChange={(e) => updateRespondent('alias', e.target.value)} />
            <FormInput id="field-rContact" label="Contact Number" placeholder="09XX XXX XXXX" inputMode="numeric" maxLength={11} value={respondent.contact} onChange={(e) => updateRespondent('contact', e.target.value.replace(/\D/g, ''))} />
            <FormInput id="field-rAge" label="Age" placeholder="Age" inputMode="numeric" maxLength={3} value={respondent.age} onChange={(e) => updateRespondent('age', e.target.value.replace(/\D/g, '').slice(0, 3))} />
            <FormInput label="Date of Birth" type="date" value={respondent.dob} onChange={(e) => updateRespondent('dob', e.target.value)} />
          </FormRow>

          <FormRow cols={3}>
            <FormSelect label="Gender" options={GENDER_OPTIONS} placeholder="Select Gender" value={respondent.gender} onChange={(e) => updateRespondent('gender', e.target.value)} />
            <FormSelect label="Civil Status" options={CIVIL_STATUS_OPTIONS} placeholder="Select Civil Status" value={respondent.civilStatus} onChange={(e) => updateRespondent('civilStatus', e.target.value)} />
            <FormSelect id="field-rRelationship" label="Relationship to Complainant" required options={RELATIONSHIP_OPTIONS} placeholder="Select Relationship" value={relationshipTypeName} onChange={(e) => { setRelationshipTypeName(e.target.value); clearErr('rRelationship') }} error={errors.rRelationship} />
          </FormRow>

          <FormInput id="field-rAddress" label="Complete Address" placeholder="House No., Street, Barangay, Municipality/City, Province" value={respondent.address} onChange={(e) => updateRespondent('address', e.target.value)} />

          <div className="flex flex-col gap-1">
            <label className="text-sm font-semibold text-slate-700 tracking-wide">Currently Living with Complainant?</label>
        ...
        <SectionCard step="D" title="Incident Details">
          <FormRow cols={3}>
            <FormSelect id="field-natureOfComplaintId" label="Nature of Complaint" required options={NATURE_OF_COMPLAINT_OPTIONS} placeholder="Select Nature of Complaint" value={natureOfComplaintId} onChange={(e) => { setNatureOfComplaintId(e.target.value); clearErr('natureOfComplaintId') }} error={errors.natureOfComplaintId} />
            <FormInput id="field-dateOfIncident" label="Date of Incident" required type="date" max={todayISO} value={incident.dateOfIncident} onChange={(e) => { updateIncident('dateOfIncident', e.target.value); clearErr('dateOfIncident') }} error={errors.dateOfIncident} />
            <FormInput label="Time of Incident" type="time" value={incident.timeOfIncident} onChange={(e) => updateIncident('timeOfIncident', e.target.value)} />
          </FormRow>
          <FormRow cols={3}>
            <FormInput id="field-placeOfIncident" label="Place / Location of Incident" required placeholder="e.g. Residence, Public Market" value={incident.placeOfIncident} onChange={(e) => { updateIncident('placeOfIncident', e.target.value); clearErr('placeOfIncident') }} error={errors.placeOfIncident} />
            <FormSelect label="Frequency of Incident" placeholder="Select Frequency" options={FREQUENCY_OPTIONS} value={incident.frequencyOfIncident} onChange={(e) => updateIncident('frequencyOfIncident', e.target.value)} />
            <FormInput label="Description of Injuries / Damages" placeholder="If any physical injuries or property damage" value={incident.descriptionOfInjuries} onChange={(e) => updateIncident('descriptionOfInjuries', e.target.value)} />
          </FormRow>
        </SectionCard>
        ...
        <SectionCard step="E" title="Type of Violence">
          <div id="field-violence" className="flex flex-col gap-2">
            <p className="text-xs text-gray-500 -mt-1">Select all types of violence that apply to this complaint.</p>
          {isLoadingOptions ? (
            <p className="text-sm text-gray-400">Loading violence types...</p>
          ) : violenceOptions.length === 0 ? (
            <p className="text-sm text-gray-500">No violence options were returned by the API.</p>
          ) : (
            <div className="grid grid-cols-2 gap-2">
              {violenceOptions.map((option) => (
                <ToggleCard key={option.id} checked={selectedViolenceIds.includes(option.id)} label={option.type} onChange={() => toggleViolence(option.id)} />
              ))}
            </div>
          )}
          {errors.violence && <p className="text-xs text-red-500 mt-1">{errors.violence}</p>}
          </div>
        </SectionCard>
        ...
        <SectionCard step="G" title="Evidence Provided">
          ...
          {isLoadingOptions ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-2">
                {(showAllEvidence ? evidenceOptions : evidenceOptions.slice(0, 8)).map((option) => {
                  const optionId = String(option.id)
                  return (
                    <ToggleCard key={option.id} checked={selectedEvidenceIds.includes(optionId)} label={option.typName} onChange={() => toggleEvidence(optionId)} />
                  )
                })}
              </div>
          ...
          <FormInput label="Other / Specify" placeholder="e.g. Medical certificate, CCTV footage, etc." />
        </SectionCard>
            ...
            {witnesses.map((w, i) => (
              <div key={i} className="border border-gray-200 rounded-lg p-4 flex flex-col gap-3 bg-gray-50/50">
                <div className="flex items-center justify-between">
                  ...
                  {witnesses.length > 1 && (
                    <button type="button" onClick={() => removeWitness(i)} className="text-xs text-gray-400 hover:text-red-500 transition-colors flex items-center gap-1">
                ...
                <PersonSearchField
                  ...
                  onSelect={(person) => {
                    ...
                    updateWitness(i, 'fullName', fullName)
                ...
                <FormRow cols={2}>
                  <div>
                    <FormInput label="Full Name" placeholder="Full name" value={w.fullName} maxLength={80} onChange={(e) => updateWitness(i, 'fullName', e.target.value)} />
                    <CharCounter current={w.fullName.length} max={80} />
                  </div>
                ...
                <div>
                  <FormInput label="Address" placeholder="Complete address" value={w.address} maxLength={180} onChange={(e) => updateWitness(i, 'address', e.target.value)} />
                ...
                <FormTextarea
                  ...
                  value={w.testimony}
                  onChange={(e) => updateWitness(i, 'testimony', e.target.value)}
                  hint="Max 500 characters"
                />
              </div>
            ))}
          </div>
          <button type="button" onClick={addWitness} className="flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 font-medium transition-colors self-start">
          ...
          <div id="field-certified">
            ...
            >
              ...
              <p className="text-sm text-gray-700 leading-relaxed">
                ...
                I understand that any false statement made herein may subject me to the penalties prescribed by law.
                I am voluntarily executing this complaint and authorize the Barangay to take appropriate action in accordance with RA 9262 (Anti-Violence Against Women and Their Children Act).
              </p>
            </label>
