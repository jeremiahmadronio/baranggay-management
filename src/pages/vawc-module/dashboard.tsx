...
const DISTRIBUTION_COLORS = ['#64748B', '#3B82F6', '#0F766E', '#7C3AED', '#D97706', '#0891B2', '#475569'];
...
function getStatusTone(status: string) {
  ...
  if (normalized.includes('PENDING')) return 'bg-amber-50 text-amber-700 border border-amber-300';
  if (normalized.includes('SETTLED') || normalized.includes('RESOLVED')) return 'bg-emerald-50 text-emerald-700 border border-emerald-300';
  if (normalized.includes('ACTIVE') || normalized.includes('ONGOING') || normalized.includes('FILED')) return 'bg-blue-50 text-blue-700 border border-blue-300';
  if (normalized.includes('WITHDRAWN')) return 'bg-orange-50 text-orange-700 border border-orange-300';
  if (normalized.includes('CLOSED')) return 'bg-slate-50 text-slate-700 border border-slate-300';
  if (normalized.includes('EXPIR')) return 'bg-red-50 text-red-700 border border-red-300';
...
export default function VAWCDashboard() {
  ...
  const [distribution, setDistribution] = useState<DashboardCaseDistributionDTO[]>([]);
    ...
    <div className="min-h-screen bg-slate-50/70">
      ...
      <KPIGrid columns={4}>
        <KPICard
          ...
          subtitle="All recorded VAWC cases"
        />
        <KPICard
          title="Active BPOs"
        ...
        <KPICard
          ...
          subtitle="Protection orders released"
        />
        <KPICard
          title="Settled Cases"
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
        <div className="overflow-hidden rounded-lg border border-gray-200 bg-white lg:col-span-8">
          <div className="flex items-center justify-between border-b border-gray-200 p-5">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Latest VAWC case records</p>
            </div>
            <button
            ...
            {loading ? (
              ...
              <table className="w-full text-left text-sm">
                <thead className="bg-gray-50 border-b border-gray-200">
                  <tr>
                    <th className="px-6 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Case No.</th>
                ...
                <tbody className="divide-y divide-gray-100">
                  {recentCases.map((item) => (
                    <tr key={item.caseNumber} className="hover:bg-slate-50/70 transition-colors">
                      <td className="px-6 py-4 font-medium text-gray-600">{item.caseNumber}</td>
                      <td className="px-6 py-4 text-gray-900 font-semibold">{item.complainantName}</td>
                      <td className="px-6 py-4 text-gray-600">{item.natureOfComplaint}</td>
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-[11px] font-semibold uppercase tracking-wide ${getStatusTone(item.status)}`}>
        ...
        <div className="flex flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm shadow-slate-200/40 lg:col-span-4">
          <div className="border-b border-slate-200 p-6">
            <h3 className="text-xl font-semibold text-gray-900">Case Distribution</h3>
            <p className="text-sm text-gray-500 mt-1">Breakdown by complaint category</p>
          </div>
          {distribution.length === 0 ? (
            <div className="p-6"><NoRecords text="No case distribution data." /></div>
          ) : (
                  ...
                  <PieChart>
                    ...
                    >
                      {distribution.map((item, index) => (
                        <Cell key={`dist-${index}`} fill={getDistributionColor(item.label, index)} />
                      ))}
                    </Pie>
                    <Tooltip
                      contentStyle={{ borderRadius: 10, border: '1px solid #E5E7EB', fontSize: 12, boxShadow: '0 8px 20px -12px rgb(15 23 42 / 0.25)' }}
                      formatter={(value: number | string | undefined) => typeof value === 'number' ? value.toLocaleString() : (value ?? '0')}
                    />
                  </PieChart>
                </ResponsiveContainer>
              </div>
                ...
                {distribution.map((item, index) => (
                  <div key={`${item.label}-${index}`} className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ backgroundColor: getDistributionColor(item.label, index) }} />
                      <p className="text-xs text-gray-700 truncate">{item.label}</p>
                    </div>
                    <span className="text-xs text-gray-900 font-medium shrink-0">{item.value.toLocaleString()}</span>
                  </div>
                ))}
              </div>
            </div>
